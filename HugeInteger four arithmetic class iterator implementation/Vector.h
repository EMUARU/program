...
public:
   ...
   vector& operator=( const vector &right )//---------------------------------------
   {
      ...
      {
          resize(right.myData.myLast - right.myData.myFirst);

          for (int i = 0; i < (right.myData.myLast - right.myData.myFirst); i++)
...
bool operator==( const vector< Ty > &left, const vector< Ty > &right )//---------------------------
{
    ...
    for (int i(0); i < left.size() && left.begin()[i] == right.begin()[i]; i++)
        if (i == left.size() - 1)
            return true;
